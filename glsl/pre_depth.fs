...
bool is_outside(float d) { return (d < cv_min_ds) || (d > cv_max_ds); }

float normalize_depth(float depth) { return (depth - cv_min_ds) / (cv_max_ds - cv_min_ds); }
...
void main(void)
{
    ...
    out_Color = vec4(rgb_to_lab(get_color(vec3(pass_TexCoord, (depth_norm <= 0.0 || depth_norm >= 1.0) ? 1.0 : depth_norm))), 1.0f);
#if 0
