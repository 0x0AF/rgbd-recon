...
std::shared_ptr<kinect::ReconMC> g_recon_mc{};
std::vector<std::shared_ptr<kinect::Reconstruction>> g_recons; // 5
...
void init_stereo_camera()
{
    ...
    g_stereo_camera = new gloost::StereoCamera(eye_matrix, 0.2, 20.0, 0.064 /*eyesep*/, screen_matrix, g_screenWidthReal, g_screenHeightReal);
}
...
void init(std::vector<std::string> const &args)
{
    ...
    g_recon_mc = std::make_shared<kinect::ReconMC>(*g_calib_files, g_cv.get(), g_bbox, g_tsdf_limit, g_voxel_size);
    g_recons.emplace_back(g_recon_mc);
...
void draw3d(void)
{
    ...
    { // ANAGLYPH STEREO
        ...
        update_model_matrix(false);
        // g_recon_mc->setColorMaskMode(2);
        g_recons.at(g_recon_mode)->setColorMaskMode(2);
        g_recons.at(g_recon_mode)->drawF();
    }
    else if(g_stereo_mode == 2 && g_fbr)
    { // SIDE-BY-SIDE STEREO
        ...
        // g_recon_mc->setViewportOffset((float) g_left_pos_x, (float) g_left_pos_y);
        ...
        glMatrixMode(GL_MODELVIEW);
        glMultMatrixf(model_mat.data());
        // g_recon_mc->setViewportOffset((float) g_right_pos_x, (float) g_right_pos_y);
...
//////////////////////////////////////////////////////////////////////////////////////////
void mouse_callback(GLFWwindow *window, double xpos, double ypos) { g_navi.motion(xpos, ypos); }
...
void quit(int status)
{
    if(g_loaded_conf)
    {
        ...
        file_name << g_conf_file.substr(0, g_conf_file.length() - 5) << "," << (now->tm_year + 1900) << '-' << (now->tm_mon + 1) << '-' << now->tm_mday << ',' << now->tm_hour << '-' << now->tm_min
                  << ".csv";
