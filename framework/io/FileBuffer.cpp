...
namespace sys
{
FileBuffer::FileBuffer(const char *path) : m_path(path), m_file(0), m_buffer(0), m_bytes_r(0), m_bytes_w(0), m_fstat(), m_looping(false) {}
...
bool FileBuffer::isOpen() { return m_file != 0; }

bool FileBuffer::open(const char *mode, unsigned buffersize)
{
...
unsigned FileBuffer::calcNumFrames(unsigned framesize) { return m_fstat.st_size / framesize; }
...
unsigned FileBuffer::write(void *buffer, unsigned numbytes)
{
    if(0 == m_file)
        return 0;
...
unsigned FileBuffer::numBytesR() const { return m_bytes_r; }

unsigned FileBuffer::numBytesW() const { return m_bytes_w; }

void FileBuffer::gotoByte(size_t offset)
{
