...
NetKinectArray::NetKinectArray(std::string const &serverport, CalibrationFiles const *calibs, CalibVolumes const *vols, bool readfromfile)
    : m_numLayers(0), m_colorArray(),
      m_depthArray_raw(), m_textures_depth{globjects::Texture::createDefault(GL_TEXTURE_2D_ARRAY)}, m_textures_depth_b{globjects::Texture::createDefault(GL_TEXTURE_2D_ARRAY)},
      ...
      m_textures_quality{globjects::Texture::createDefault(GL_TEXTURE_2D_ARRAY)}, m_textures_normal{globjects::Texture::createDefault(GL_TEXTURE_2D_ARRAY)},
      ...
      m_out_rgbs{globjects::Texture::createDefault(GL_TEXTURE_3D)}, m_out_depths{globjects::Texture::createDefault(GL_TEXTURE_3D)}, m_out_silhouettes{globjects::Texture::createDefault(GL_TEXTURE_3D)}
{
    ...
    m_programs.at("filter")->attach(globjects::Shader::fromFile(GL_VERTEX_SHADER, "glsl/texture_passthrough.vs"), globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "glsl/pre_depth.fs"));
    m_programs.at("normal")->attach(globjects::Shader::fromFile(GL_VERTEX_SHADER, "glsl/texture_passthrough.vs"), globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "glsl/pre_normal.fs"));
    m_programs.at("quality")->attach(globjects::Shader::fromFile(GL_VERTEX_SHADER, "glsl/texture_passthrough.vs"), globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "glsl/pre_quality.fs"));
    m_programs.at("boundary")->attach(globjects::Shader::fromFile(GL_VERTEX_SHADER, "glsl/texture_passthrough.vs"), globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "glsl/pre_boundary.fs"));
    m_programs.at("morph")->attach(globjects::Shader::fromFile(GL_VERTEX_SHADER, "glsl/texture_passthrough.vs"), globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "glsl/pre_morph.fs"));
}

bool NetKinectArray::init()
{
    ...
    if(m_calib_files->isCompressedRGB() == 1)
    {
        ...
        m_colorArray = std::unique_ptr<TextureArray>{
            new TextureArray(m_resolution_color.x, m_resolution_color.y, m_numLayers, GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, GL_UNSIGNED_BYTE, m_colorsize)};
    }
    else if(m_calib_files->isCompressedRGB() == 5)
    {
        std::cout << "Color DXT 5 compressed" << std::endl;
        m_colorArray = std::unique_ptr<TextureArray>{
            new TextureArray(m_resolution_color.x, m_resolution_color.y, m_numLayers, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, GL_UNSIGNED_BYTE, m_colorsize)};
    }
    else
    {
...
NetKinectArray::~NetKinectArray()
{
    ...
    m_colorArray->unbind();
    m_colorArray_back->unbind();
    m_depthArray_raw->unbind();

    m_fbo->destroy();

    m_textures_bg.front->destroy();
    m_textures_bg.back->destroy();

    m_textures_depth2.front->destroy();
    m_textures_depth2.back->destroy();

    m_textures_depth->destroy();
    m_textures_color->destroy();
...
bool NetKinectArray::update()
{
    ...
    if(!m_pbo_colors.dirty || !m_pbo_depths.dirty)
        return false;
...
glm::uvec2 NetKinectArray::getDepthResolution() const { return m_resolution_depth; }
glm::uvec2 NetKinectArray::getColorResolution() const { return m_resolution_color; }

int NetKinectArray::getTextureUnit(std::string const &name) const { return m_texture_unit_offsets.at(name); }
...
unsigned NetKinectArray::getStartTextureUnit() const { return m_start_texture_unit; }
...
void NetKinectArray::writeCurrentTexture(std::string prefix)
{
    // depths
    if(m_calib_files->isCompressedDepth())
    {
...
void NetKinectArray::writeBMP(std::string filename, std::vector<std::uint8_t> const &data, unsigned int offset, unsigned int bytesPerPixel)
{
    ...
    c = 'M';
    file.write(&c, 1);
    i = m_resolution_color.x * m_resolution_color.y * 3 + 54;
    ...
    i = 40;
    file.write((char const *)&i, 4);
    i = m_resolution_color.x;
    file.write((char const *)&i, 4);
    i = m_resolution_color.y;
    file.write((char const *)&i, 4);
    s = 1;
    ...
    i = 0;
    file.write((char const *)&i, 4);
    i = m_resolution_color.x * m_resolution_color.y * 3;
    file.write((char const *)&i, 4);
    i = 0;
    file.write((char const *)&i, 4);
    i = 0;
...
void NetKinectArray::readFromFiles()
{
