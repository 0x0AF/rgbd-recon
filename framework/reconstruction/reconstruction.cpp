...
Reconstruction::Reconstruction(CalibrationFiles const &cfs, CalibVolumes const *cv, gloost::BoundingBox const &bbox)
    : m_cv(cv), m_cf(&cfs), m_tex_width{cfs.getWidth()}, m_tex_height{cfs.getHeight()}, m_num_kinects{cfs.num()}, m_min_length{cfs.minLength()}, m_bbox{bbox}, m_color_mask_mode(0)
{
...
void Reconstruction::drawComparison() { draw(); }
...
void Reconstruction::setColorMaskMode(unsigned mode) { m_color_mask_mode = mode; }
void Reconstruction::setViewportOffset(float x, float y) { std::cerr << "Reconstruction::setViewportOffset(float x, float y) -> implement me in derived class!" << std::endl; }
