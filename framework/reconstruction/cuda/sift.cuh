...
//********************************************************//
// CUDA SIFT extractor by M?rten Bj?rkman aka Celebrandil //
//********************************************************//

/*
 * Compacted for mobility
 *
 * xAF
 */
...
void ScaleUp(CudaImage &res, CudaImage &src);
void ComputeOrientations(cudaTextureObject_t texObj, SiftData &siftData, int fstPts, int totPts);
void ExtractSiftDescriptors(cudaTextureObject_t texObj, SiftData &siftData, int fstPts, int totPts, float subsampling);
void RescalePositions(SiftData &siftData, float scale);
void LowPass(CudaImage &res, CudaImage &src, float scale);
void LaplaceMulti(cudaTextureObject_t texObj, CudaImage &baseImage, CudaImage *results, float baseBlur, float diffScale, float initBlur);
void FindPointsMulti(CudaImage *sources, SiftData &siftData, float thresh, float edgeLimit, float scale, float factor, float lowestScale, float subsampling);
...
void FindHomography(SiftData &data, float *homography, int *numMatches, int numLoops = 1000, float minScore = 0.85f, float maxAmbiguity = 0.95f, float thresh = 5.0f);
...
///////////////////////////////////////////////////////////////////////////////
__global__ void KernelScaleDown(float *d_Result, float *d_Data, int width, int pitch, int height, int newpitch)
{
...
__global__ void KernelScaleUp(float *d_Result, float *d_Data, int width, int pitch, int height, int newpitch)
{
...
#define BH (SCALEUP_H / 2 + 2)
    ...
    if(x < 2 * width && y < 2 * height)
    {
        ...
        d_Result[y * newpitch + x] = wy * (wx * buffer[bp] + (1.0f - wx) * buffer[bp + 1]) + (1.0f - wy) * (wx * buffer[bp + BW] + (1.0f - wx) * buffer[bp + BW + 1]);
    }
}

__global__ void KernelExtractSiftDescriptors(cudaTextureObject_t texObj, SiftPoint *d_sift, int fstPts, float subsampling)
{
    ...
    for(int y = ty; y < 16; y += 8)
    {
        ...
        float dx = tex2D<float>(texObj, xpos + cosa, ypos + sina) - tex2D<float>(texObj, xpos - cosa, ypos - sina);
        float dy = tex2D<float>(texObj, xpos - sina, ypos + cosa) - tex2D<float>(texObj, xpos + sina, ypos - cosa);
...
__global__ void KernelExtractSiftDescriptorsOld(cudaTextureObject_t texObj, SiftPoint *d_sift, int fstPts, float subsampling)
{
    __shared__ float gauss[16];
    __shared__ float buffer[128];
...
__global__ void KernelRescalePositions(SiftPoint *d_sift, int numPts, float scale)
{
...
__global__ void KernelComputeOrientations(cudaTextureObject_t texObj, SiftPoint *d_Sift, int fstPts)
{
...
__global__ void KernelFindPointsMulti(float *d_Data0, SiftPoint *d_Sift, int width, int pitch, int height, int nScales, float subsampling, float lowestScale)
{
...
__global__ void KernelLaplaceMultiTex(cudaTextureObject_t texObj, float *d_Result, int width, int pitch, int height)
{
    ...
    float x = xp - 3.5;
    float y = yp + 0.5;
    sdata1[tx] = kernel[4] * tex2D<float>(texObj, x, y) + kernel[3] * (tex2D<float>(texObj, x, y - 1.0) + tex2D<float>(texObj, x, y + 1.0)) +
                 kernel[2] * (tex2D<float>(texObj, x, y - 2.0) + tex2D<float>(texObj, x, y + 2.0)) + kernel[1] * (tex2D<float>(texObj, x, y - 3.0) + tex2D<float>(texObj, x, y + 3.0)) +
                 kernel[0] * (tex2D<float>(texObj, x, y - 4.0) + tex2D<float>(texObj, x, y + 4.0));
    __syncthreads();
    float *sdata2 = data2 + LAPLACE_W * scale;
    if(tx < LAPLACE_W)
    {
        sdata2[tx] = kernel[4] * sdata1[tx + 4] + kernel[3] * (sdata1[tx + 3] + sdata1[tx + 5]) + kernel[2] * (sdata1[tx + 2] + sdata1[tx + 6]) + kernel[1] * (sdata1[tx + 1] + sdata1[tx + 7]) +
...
__global__ void KernelLaplaceMultiMem(float *d_Image, float *d_Result, int width, int pitch, int height)
{
    __shared__ float data1[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
    __shared__ float data2[LAPLACE_W * LAPLACE_S];
    const int tx = threadIdx.x;
    ...
    int h = height - 1;
    sdata1[tx] = kernel[4] * data[min(yp, h) * pitch] + kernel[3] * (data[max(0, min(yp - 1, h)) * pitch] + data[min(yp + 1, h) * pitch]) +
                 kernel[2] * (data[max(0, min(yp - 2, h)) * pitch] + data[min(yp + 2, h) * pitch]) + kernel[1] * (data[max(0, min(yp - 3, h)) * pitch] + data[min(yp + 3, h) * pitch]) +
...
__global__ void KernelLowPass(float *d_Image, float *d_Result, int width, int pitch, int height)
{
    ...
    int h = height - 1;
    if(yp < height)
        buff[tx] = kernel[4] * data[min(yp, h) * pitch] + kernel[3] * (data[max(0, min(yp - 1, h)) * pitch] + data[min(yp + 1, h) * pitch]) +
                   kernel[2] * (data[max(0, min(yp - 2, h)) * pitch] + data[min(yp + 2, h) * pitch]) + kernel[1] * (data[max(0, min(yp - 3, h)) * pitch] + data[min(yp + 3, h) * pitch]) +
                   kernel[0] * (data[max(0, min(yp - 4, h)) * pitch] + data[min(yp + 4, h) * pitch]);
    __syncthreads();
    if(tx < LOWPASS_W && xp < width && yp < height)
    {
        d_Result[yp * pitch + xp] = kernel[4] * buff[tx + 4] + kernel[3] * (buff[tx + 3] + buff[tx + 5]) + kernel[2] * (buff[tx + 2] + buff[tx + 6]) + kernel[1] * (buff[tx + 1] + buff[tx + 7]) +
...
{
    if(data.h_data != NULL || data.d_data != NULL)
        FreeSiftData(data);
    data.numPts = 0;
    data.maxPts = num;
...
void ScaleDown(CudaImage &res, CudaImage &src, float variance)
{
    ...
    KernelScaleDown<<<blocks, threads>>>(res.d_data, src.d_data, src.width, src.pitch, src.height, res.pitch);
...
void ScaleUp(CudaImage &res, CudaImage &src)
{
    ...
    KernelScaleUp<<<blocks, threads>>>(res.d_data, src.d_data, src.width, src.pitch, src.height, res.pitch);
...
void ComputeOrientations(cudaTextureObject_t texObj, SiftData &siftData, int fstPts, int totPts)
{
    ...
    dim3 threads(128);
#ifdef MANAGEDMEM
    KernelComputeOrientations<<<blocks, threads>>>(texObj, siftData.m_data, fstPts);
#else
    KernelComputeOrientations<<<blocks, threads>>>(texObj, siftData.d_data, fstPts);
#endif
...
void ExtractSiftDescriptors(cudaTextureObject_t texObj, SiftData &siftData, int fstPts, int totPts, float subsampling)
{
    ...
    dim3 threads(16, 8);
#ifdef MANAGEDMEM
    KernelExtractSiftDescriptors<<<blocks, threads>>>(texObj, siftData.m_data, fstPts, subsampling);
#else
    KernelExtractSiftDescriptors<<<blocks, threads>>>(texObj, siftData.d_data, fstPts, subsampling);
#endif
...
void RescalePositions(SiftData &siftData, float scale)
{
    ...
    dim3 threads(64);
    KernelRescalePositions<<<blocks, threads>>>(siftData.d_data, siftData.numPts, scale);
...
void LowPass(CudaImage &res, CudaImage &src, float scale)
{
    ...
    KernelLowPass<<<blocks, threads>>>(src.d_data, res.d_data, width, pitch, height);
...
#if 1
    KernelLaplaceMultiMem<<<blocks, threads>>>(baseImage.d_data, results[0].d_data, width, pitch, height);
#else
    KernelLaplaceMultiTex<<<blocks, threads>>>(texObj, results[0].d_data, width, pitch, height);
#endif
...
#ifdef MANAGEDMEM
    KernelFindPointsMulti<<<blocks, threads>>>(sources->d_data, siftData.m_data, w, p, h, NUM_SCALES, subsampling, lowestScale);
#else
    KernelFindPointsMulti<<<blocks, threads>>>(sources->d_data, siftData.d_data, w, p, h, NUM_SCALES, subsampling, lowestScale);
#endif
