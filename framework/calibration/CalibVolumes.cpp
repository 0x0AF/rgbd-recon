...
CalibVolumes::CalibVolumes(std::vector<std::string> const &calib_volume_files, gloost::BoundingBox const &bbox)
    : m_cv_xyz_filenames(), m_cv_uv_filenames(), m_volumes_xyz{}, m_volumes_uv{}, m_volumes_xyz_inv{}, m_buffer_bbox{new globjects::Buffer()}, m_frustums{}, m_bbox{bbox}, m_start_texture_unit(-1),
...
void CalibVolumes::loadInverseCalibs(std::string const &path)
{
    for(unsigned i = 0; i < m_cv_xyz_filenames.size(); ++i)
    {
        ...
        std::cout << "dimensions xyz - " << calib.res().x << ", " << calib.res().y << ", " << calib.res().z << " minmax d - " << calib.depthLimits().x << ", " << calib.depthLimits().y << std::endl;
    }
...
void CalibVolumes::addVolume(std::string const &filename_xyz, std::string const &filename_uv)
{
    ...
    std::cout << "dimensions xyz - " << calib_xyz.res().x << ", " << calib_xyz.res().y << ", " << calib_xyz.res().z << " minmax d - " << calib_xyz.depthLimits().x << ", " << calib_xyz.depthLimits().y
              << std::endl;
    ...
    std::cout << "dimensions uv - " << calib_uv.res().x << ", " << calib_uv.res().y << ", " << calib_uv.res().z << " minmax d - " << calib_uv.depthLimits().x << ", " << calib_uv.depthLimits().y
              << std::endl;
}
...
void CalibVolumes::bindToTextureUnits()
{
...
void CalibVolumes::bindToTextureUnitsInv()
{
    for(unsigned layer = 0; layer < m_cv_xyz_filenames.size(); ++layer)
    {
...
Frustum const &CalibVolumes::getFrustum(unsigned i) const { return m_frustums[i]; }
...
const std::vector<globjects::Texture *> &CalibVolumes::getVolumesXYZ () const { return m_volumes_xyz; }
